...
class FrameProcessor:
    ...
    def __init__(self, input_path: str, sequence_fps: int = 25):
        """
        Initialize the frame processor.
        
        Args:
            input_path: Path to the input video file or directory of frames
            sequence_fps: FPS to use when processing image sequences
        """
        ...
        if os.path.isdir(input_path):
            ...
            # For directories, we'll get dimensions from the first frame
            import glob
            frame_files = sorted(glob.glob(os.path.join(input_path, '*.jpg'))) + \
            ...
            if first_frame is None:
                raise ValueError(f"Could not read image file: {frame_files[0]}")
                
            self.height, self.width = first_frame.shape[:2]
            self.fps = sequence_fps
        else:
            # Traditional video file handling
    ...
    def process_video(self, output_path: str):
        ...
        """
        # Create frame generator based on input type
        if self.is_directory:
            import os
            import glob
            
            # Get all image files in the directory
            frame_files = sorted(glob.glob(os.path.join(self.input_path, '*.jpg'))) + \
                          sorted(glob.glob(os.path.join(self.input_path, '*.png')))
            
            if not frame_files:
                raise ValueError(f"No image files found in directory: {self.input_path}")
